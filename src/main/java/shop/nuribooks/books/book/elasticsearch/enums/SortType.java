...
public enum SortType {
	...
	// 구현 예정
	POPULAR(SortOptions.of(so -> so.score(ss -> ss.order(SortOrder.Desc)))),
	...
	STAR(SortOptions.of(so -> so.score(ss -> ss.order(SortOrder.Desc)))),
	REVIEW_COUNT(SortOptions.of(so -> so.score(ss -> ss.order(SortOrder.Desc))));
